...
// Моменты времени для вывода (дни → секунды)
...
/**
 * Метод прогонки (Thomas algorithm) для трехдиагональной СЛАУ
 *
...
/**
 * Сохранение профиля давления в файл
 */
...
int main() {
    ...
    for (int step = 1; step < M; ++step) {
        ...
        // ГРАНИЧНЫЕ УСЛОВИЯ
        // --------------------------------------------------------------------
        // i = 0: p = P_LEFT (нагнетание)
        a[0] = 1.0;
        b[0] = 0.0;  // не используется
        d[0] = P_LEFT;

        // i = N-1: p = P_RIGHT (добыча)
        a[N-1] = 1.0;
        c[N-2] = 0.0;  // не используется
        d[N-1] = P_RIGHT;
        ...
        for (int i = 1; i < N - 1; ++i) {
            // === UPWIND-АППРОКСИМАЦИЯ ПЛОТНОСТИ ===
    ...
    std::cout << "Сохранено профилей: " << profile_count << std::endl;
    std::cout << "Результаты в папке: output/" << std::endl;

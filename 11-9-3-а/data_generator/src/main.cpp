...
/**
 * @brief Метод Ньютона для решения нелинейного уравнения r(α) = y(1; α) - 2 = 0
 * 
 * @details 
 * Реализует метод Ньютона (касательных) для нахождения корня уравнения:
 *   r(α) = y(1; α) - 2 = 0
 * где y(1; α) - значение решения краевой задачи в точке x=1 при начальном условии y'(0) = α.
 * 
 * Метод использует итерационную формулу:
 *   α_{k+1} = α_k - r(α_k) / r'(α_k)
 * где:
 *   r(α_k) = y(1; α_k) - 2 - невязка (разность между полученным и желаемым значением)
 *   r'(α_k) = ∂y(1; α_k)/∂α - производная невязки, вычисляемая через уравнения чувствительности
 * 
 * Производная r'(α) вычисляется АНАЛИТИЧЕСКИ через решение сопряженной системы уравнений
 * чувствительности, что обеспечивает квадратичную сходимость метода.
 * 
 * @param alpha0 Начальное приближение для параметра α = y'(0). 
 *               Должно быть достаточно близко к истинному значению для сходимости.
 * @param n Количество шагов интегрирования на отрезке [0, 1]. 
 *          Влияет на точность вычисления y(1) и ∂y(1)/∂α.
 *          Рекомендуется: n >= 1000 для точности 1e-8.
 * @param tol Допустимая погрешность |r(α)| = |y(1) - 2|.
 *            Итерации прекращаются при |y(1) - 2| < tol.
 *            По умолчанию: 1e-8.
 * @param max_iter Максимальное количество итераций метода Ньютона.
 *                 Защита от бесконечного цикла при расходимости.
 *                 По умолчанию: 50.
 * 
